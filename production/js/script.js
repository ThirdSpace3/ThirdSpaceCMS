...
function openTab(evt, tabName) {
  ...
  if (evt) {
    ...
    // If no event is provided (like on initial load), manually set the first tab link to active
    document.getElementsByClassName("tablinks")[0].className += " active";
  }
}

// Function to open the default tab and set its button to active on load
function openDefaultTab() {
  // Open the first tab 'TabA' and set its button to active on load
  openTab(null, 'TabA');
}

// Add the event listener to open the default tab when the page loads
